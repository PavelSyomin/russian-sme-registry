...
def test_panelize(tmp_path):
    ...
    out_file = tmp_path / "entire-dataset/csv/all-regions.csv"
...
def test_panelize_save_to_parquet(tmp_path):
    ...
    revexp_file = pathlib.Path(__file__).parent / "data/revexp/test-aggregated.csv"
    empl_file = pathlib.Path(__file__).parent / "data/empl/test-aggregated.csv"
    out_file = tmp_path / "entire-dataset/parquet/all-regions.parquet"
...
def test_panelize_save_to_excel(tmp_path):
    ...
    revexp_file = pathlib.Path(__file__).parent / "data/revexp/test-aggregated.csv"
    empl_file = pathlib.Path(__file__).parent / "data/empl/test-aggregated.csv"
    out_file = tmp_path / "entire-dataset/excel/all-regions.xlsx"
...
def test_panelize_make_filtered_orgs_table(tmp_path):
    sme_file = pathlib.Path(__file__).parent / "data/sme/test-geocoded.csv"
    revexp_file = pathlib.Path(__file__).parent / "data/revexp/test-aggregated.csv"
    empl_file = pathlib.Path(__file__).parent / "data/empl/test-aggregated.csv"
    out_file_1 = tmp_path / "entire-dataset/csv/all-regions.csv"
    out_file_2 = tmp_path / "filtered-orgs/csv/all-regions.csv"
    ...
    panelizer(
        ...
        str(revexp_file),
        str(empl_file),
        make_filtered_orgs_table=True,
    )

    df = pd.read_csv(out_file_1, dtype=str, sep=";")
    ...
    df = pd.read_csv(out_file_2, dtype=str, sep=";")
